...
Note that [different units are available](https://pyafscgap.org/devdocs/afscgap.html#data-structure) and can be passed to `get_cpue_weight`. Also, in this example, only records with Pacific cod reported are included ("presence-only" data). See zero catch inference below. In other words, it reports on CPUE only for hauls in which Pacific cod were recorded, excluding some hauls like those in which Pacific cod were not found at all.
...
### Load into Pandas
The results from the `afscgap` package are serializable and can be loaded into other tools like [Pandas](https://pandas.pydata.org/). This example will load Pacific cod from 2021 Gulf of Alaska into a data frame.
...
This example revisits the earlier example but `set_presence_only(False)` directs the library to look at data on hauls to determine which hauls did not have Pacific cod. This lets the library return records for hauls in which Pacific cod were not found. Therefore, while the earlier example showed CPUE counts for hauls in which Pacific cod were seen, this revised example reports for all hauls in the Gulf of Alaska in 2021.
