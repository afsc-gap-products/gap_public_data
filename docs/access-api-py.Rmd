...
This example revisits the earlier example but `set_presence_only(False)` directs the library to look at data on hauls to determine which hauls did not have Pacific cod. This lets the library return records for hauls in which Pacific cod were not found. This can be seen in differences in counts reported:
