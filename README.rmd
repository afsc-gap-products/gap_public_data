...
Pull the `RACEBASE_FOSS.JOIN_FOSS_CPUE_COMB`, `RACEBASE_FOSS.JOIN_FOSS_CPUE_CATCH`, and  `RACEBASE_FOSS.JOIN_FOSS_CPUE_HAUL` tables, which are much smaller, and combine the table locally yourself. 

`r paste(readLines(con = paste0(dir_out, "JOIN_FOSS_CPUE_table_metadata.txt")), collapse="\n") `

```{r oracle-data-size-join-comb, echo = FALSE}
str0 <- paste0(
  "RACEBASE_FOSS.JOIN_FOSS_CPUE_COMB: ", "
  rows: ", nrow(JOIN_FOSS_CPUE_COMB), "
  cols: ", ncol(JOIN_FOSS_CPUE_COMB), "
  ", round(file.size(paste0(dir_out, "JOIN_FOSS_CPUE_COMB"))/1e+9, 3), " GB") 
cat(str0)
```
...
str0 <- paste0(
  ...
  ", round(file.size(paste0(dir_out, "JOIN_FOSS_CPUE_CATCH"))/1e+9, 3), " GB") 
cat(str0)
```
