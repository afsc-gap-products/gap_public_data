...
This is presence and absence data. This is a huge file and has all of the bells and whistles. Keep this in mind while pulling data. For reference: 
...
```{r api-subset-ex, echo = FALSE, eval = FALSE}
...
If you have access to the AFSC Oracle database, you can pull the data directly from the Oracle schema these data are pulled from for FOSS. 

### Connecting to Oracle from R

While you can access the data directly by opening `SQL developer`, many users will want to access the data using `R`. You will need to install the `RODBC` R package and have OFIS (IT) connect R to Oracle. Once connected, you can use the following code in R to connect to Oracle. 
