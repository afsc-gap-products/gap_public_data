...
design_based1 <- dplyr::bind_rows(
  ...
  dplyr::rename(latitude = start_latitude, 
                ...
                weight_kg = weight, 
                count = number_fish) %>% 
  dplyr::mutate(cpue_kgha = wgtcpue/100, # denominator in km2
                cpue_noha = numcpue/100, # denominator in km2
                area_swept_ha = effort/100) # denominator in km2
...
dat <- dplyr::full_join(
  x = data_new %>% 
    ...
    # dplyr::mutate(cpue_kgha = cpue_kgha*1e4),
  y = design_based %>% 
    dplyr::select(common_name, species_code, scientific_name, 
                  cpue_kgha, cpue_noha, 
                  area_swept_ha, distance_fished_km, net_width_m, 
...
dat0 <-dat %>% 
  ...
  dplyr::filter(cpue_kgha_diff != 0 | cpue_noha_diff != 0 | area_swept_ha_diff != 0)

diff_xnew_ydesign <- dat0[,order(colnames(dat0))]

readr::write_csv(diff_xnew_ydesign, 
